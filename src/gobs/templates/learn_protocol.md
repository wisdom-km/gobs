...
L0 对零基础讲。有论文用原文词+语境人话，不要另造正名。论文第一课只吃摘要+引言。过程课必须 ASCII，排队要把「糊」画进图。讲完先问哪步不懂，补上再课间确认（细则见 `/learn` skill）。L1 只在旧图上贴名字。

零基础未画先禁：配件、零件、有关。配件 vs 整台必须先画「排队的人 + 脸上的眼镜」左右对照；拆和留是同一节点，不要把右边推到下一课。attention 说成眼镜找这个字在说谁。讲完问哪里没听懂。听懂了/可以了只收当前 phase，不要自动开下一课。禁止对学生说最空、这课行。

encode / map 课可以打开 `80_meta/gobs-viz/画图.md` 里的离线画图页（`draw.html`）。仍是一课一个 phase，encode = 1 个组块。
