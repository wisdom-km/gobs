...
def boot_prompt(
    ...
    *,
    ...
    teach = (
        ...
        "过程课必须画出「有它 / 没有它」：过程课优先打开 gobs learn desk（对话+图+笔记同一页，不靠 Obsidian）；没有 desk 再打开 80_meta/gobs-viz/process.html 让对方自己点下一步。静态对照可用 matplotlib/graphviz（python 80_meta/gobs-viz/draw.py）或讲解里当场 Mermaid；库图这轮没好再用 ASCII 兜底。排队课要把印象包被挤画出来，不能只画箭头。定界、判断不要硬画。"
        "图是通用协议：教练按这篇论文的例子写 80_meta/gobs-viz/figure.json，不要套死小猫句（除非这篇就是这句）。教课（enough/map/principles/encode）图上先给出论文对的标，对方只点揭开，不能自己把每个词都涂成高。"
        "测验课（retrieve/feynman）对方自己标，再 gobs learn judge 或课桌交卷。每个词都高但论文不是这样，不算过。"
