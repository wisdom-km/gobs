...
class LearnTests(unittest.TestCase):
    ...
    def test_init_installs_learn_skills(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertNotIn("四列表", viz_html)
            self.assertNotIn("回教", viz_html)
            ...
            self.assertIn("80_meta/gobs-viz", agents)
            self.assertNotIn("今天看哪篇只认库根", agents)
            self.assertNotIn("## 四列表", agents)
            self.assertNotIn("## 回教", agents)
            ...
            self.assertIn("80_meta/gobs-viz", learn_text)
            self.assertIn("画图.md", learn_text)
